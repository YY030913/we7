...
CREATE TABLE IF NOT EXISTS `ims_stonefish_scratch_exchange` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '规则id',
  `uniacid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '公众号ID',
...
CREATE TABLE IF NOT EXISTS `ims_stonefish_scratch_sharedata` (
 ...
 ";
$sql_fanstmplmsg = "CREATE TABLE IF NOT EXISTS `ims_stonefish_scratch_fanstmplmsg` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `rid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '规则id',
  `uniacid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '公众号ID',
  `from_user` varchar(50) DEFAULT '0' COMMENT '用户openid',
...
if(!pdo_fieldexists('stonefish_scratch_reply', 'tips')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_scratch_reply')." ADD `tips` varchar(200) DEFAULT '' COMMENT '活动次数提示' AFTER `notprizetext`;");
}
if(!pdo_fieldexists('stonefish_scratch_reply', 'number_times_tips')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_scratch_reply')." ADD `number_times_tips` varchar(100) DEFAULT '' COMMENT '超过总次数提示' AFTER `number_times`;");
}
if(!pdo_fieldexists('stonefish_scratch_reply', 'day_number_times_tips')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_scratch_reply')." ADD `day_number_times_tips` varchar(100) DEFAULT '' COMMENT '超过每天次数提示' AFTER `day_number_times`;");
}
if(!pdo_fieldexists('stonefish_scratch_reply', 'award_num_tips')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_scratch_reply')." ADD `award_num_tips` varchar(100) DEFAULT '' COMMENT '超过中奖数量提示' AFTER `award_num`;");
}
if(!pdo_fieldexists('stonefish_scratch_exchange', 'before')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_scratch_exchange')." ADD `before` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '兑奖资料活动前还是中奖后1前2为后' AFTER `baidumaplat`;");
}else{
	pdo_run($sql_fanstmplmsg);
}
if(pdo_fieldexists('stonefish_scratch_share', 'sharenum')) {
	pdo_query("ALTER TABLE ".tablename('stonefish_scratch_share')." change `sharenum` `sharenum` varchar(5) DEFAULT '0' COMMENT '分享赠送礼盒基数';");
}