...
class Tim_stralbumModule extends WeModule {

    public function fieldsFormDisplay($rid = 0) {
        //要嵌入规则编辑页的自定义内容，这里 $rid 为对应的规则编号，新增时为 0
        global $_W, $_GPC;
        if (!empty($rid)) {
            $reply = pdo_fetchall("SELECT * FROM " . tablename('stralbum_reply') . " WHERE rid = :rid", array(':rid' => $rid));
            if (!empty($reply)) {
    ...
    public function fieldsFormSubmit($rid) {
        ...
        //删除旧的
        pdo_query("delete from ".tablename('stralbum_reply')." where rid=:rid",array(':rid'=>$rid));

        if (!empty($_GPC['albumid'])) {
            foreach ($_GPC['albumid'] as $aid) {
                pdo_insert('stralbum_reply', array(
                    'rid' => $rid,
                    'albumid' => $aid,
                ));
            }
