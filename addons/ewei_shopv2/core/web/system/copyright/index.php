		...
		if ($_W['ispost']) {
			...
			foreach ($sets as $set) {
				...
				if (($wechatid == $uniacid) || ($wechatid == -1)) {
					$cs = pdo_fetch('select * from ' . tablename('ewei_shop_system_copyright') . ' where uniacid=:uniacid and ismanage=0 limit 1', array(':uniacid' => $uniacid));
			...
			if ($wechatid == -1) {
				$global_copyrights = pdo_fetch('select * from ' . tablename('ewei_shop_system_copyright') . ' where uniacid=-1 and ismanage=0 limit 1');
