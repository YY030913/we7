...
class Index_EweiShopV2Page extends WebPage
{
	public function main()
	{
		...
		if ($goodsfrom == 'sale') {
			$condition .= ' AND g.`status` = 1  and g.`total`>0 and g.`deleted`=0';
			$status = 1;
		}
		else if ($goodsfrom == 'out') {
			$condition .= ' AND g.`total` <= 0 and g.`deleted`=0';
			$status = 1;
		}
	...
	public function status()
	{
		...
		foreach ($items as $item) {
			...
			plog('goods.edit', ('修改商品状态<br/>ID: ' . $item['id'] . '<br/>商品名称: ' . $item['title'] . '<br/>状态: ' . $_GPC['enabled']) == 1 ? '上架' : '下架');
		}
