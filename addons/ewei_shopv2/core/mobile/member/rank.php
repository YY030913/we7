...
class Rank_EweiShopV2Page extends MobilePage
{
	...
	protected function getRank($update = false)
	{
		...
		if (empty($rank_cache) || ($rank_cache['time'] < TIMESTAMP) || $update) {
			$result = pdo_fetchall('SELECT id,uid,credit1,nickname,avatar,openid FROM ' . tablename('ewei_shop_member') . ' WHERE uniacid = :uniacid', array(':uniacid' => $_W['uniacid']));
	...
	public function main()
	{
		...
		$paiming = 0;
		array_walk($result, function($val, $key) use(&$paiming, $_W) {
	...
	public function order_rank()
	{
		...
		$this->orderStatus();
		$user = m('member')->getMember($_W['openid'], true);
		$result_one = pdo_fetch('SELECT SUM(price) as price,openid FROM ' . tablename('ewei_shop_order') . ' WHERE uniacid = :uniacid AND openid = :openid AND status = 3', array(':uniacid' => $_W['uniacid'], ':openid' => $user['openid']));
