...
class Detail_EweiShopV2Page extends CreditshopMobilePage
{
	...
	public function pay($a = array(), $b = array())
	{
		...
		$log = array('uniacid' => $uniacid, 'openid' => $openid, 'logno' => m('common')->createNO('creditshop_log', 'logno', empty($goods['type']) ? 'EE' : 'EL'), 'goodsid' => $id, 'status' => 0, 'paystatus' => 0 < $goods['money'] ? 0 : -1, 'dispatchstatus' => $dispatchstatus, 'createtime' => time(), 'storeid' => intval($_GPC['storeid']), 'realname' => trim($_GPC['realname']), 'mobile' => trim($_GPC['mobile']));
