define(['jquery.ui'], function (ui) {
    ...
    modal.initNavs = function () {
        modal.getNavs();
        var navgroup = {
            0: ['notice', 'richtext', 'banner', 'title', 'search', 'line', 'blank', 'menu', 'picture', 'picturew', 'goods'],
            1: ['diymod'],
            2: ['diymod'],
            99: []
        };
    ...
    modal.initEditor = function () {
        ...
        if (goodsSelector > 0) {
            ...
            _this.attr({'id': 'goods_selector', 'data-url': biz.url('goods/query'), 'data-callback': 'callbackGoods'});
        ...
        if (goodsSelector > 0) {
            ...
            _this.attr({
                'id': 'group_selector',
                'data-url': biz.url('goods/group/query'),
    ...
    modal.initMod = function (item) {
        $.ajax(biz.url('diypage/page/mod/query'), {type: "get", dataType: "html", cache: false}).done(function (html) {
    ...
    modal.save = function (preview) {
        ...
        if (modal.type == 1) {
            if (modal.id > 0) {
                var posturl = biz.url("diypage/page/diy/edit")
            } else {
                var posturl = biz.url("diypage/page/diy/add")
            }
        } else if (modal.type > 1 && modal.type < 99) {
            if (modal.id > 0) {
                var posturl = biz.url("diypage/page/sys/edit")
            } else {
                var posturl = biz.url("diypage/page/sys/add")
            }
        } else if (modal.type == 99) {
            if (modal.id > 0) {
                var posturl = biz.url("diypage/page/mod/edit")
            } else {
                var posturl = biz.url("diypage/page/mod/add")
            }
        }
        $.post(posturl, {id: modal.id, data: modal.data}, function (ret) {
            ...
            if (pageid == modal.id) {
                ...
                if (preview) {
                    ...
                    setTimeout(function () {
                        var previewUrl = biz.url("diypage/page/preview") + "&id=" + pageid;
                        window.open(previewUrl)
                    }, 1000)
                } else {
                    tip.msgbox.suc("保存成功！")
