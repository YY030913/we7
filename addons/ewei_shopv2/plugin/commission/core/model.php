	...
	class CommissionModel extends PluginModel
	{
			...
			if (!empty($member['agentid'])) {
				...
				if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
					...
					if ($order['agentid'] == $member['id']) {
						$this->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'ordersn' => $order['ordersn'], 'price' => $pricetotal, 'goods' => $goods, 'commission1' => $commission_total1, 'commission2' => $commission_total2, 'commission3' => $commission_total3, 'paytime' => $order['paytime']), TM_COMMISSION_ORDER_PAY);
						return NULL;
					}

					if ($order['agentid'] == $parent['id']) {
						$this->sendMessage($parent['openid'], array('nickname' => $member['nickname'], 'ordersn' => $order['ordersn'], 'price' => $pricetotal, 'goods' => $goods, 'commission1' => $commission_total1, 'commission2' => $commission_total2, 'commission3' => $commission_total3, 'paytime' => $order['paytime']), TM_COMMISSION_ORDER_PAY);
					}
				}
			}
		}
		...
		public function checkOrderFinish($orderid = '')
		{
			...
			if (!empty($member['agentid'])) {
				...
				if (!empty($parent) && ($parent['isagent'] == 1) && ($parent['status'] == 1)) {
					...
					if ($order['agentid'] == $member['id']) {
						$this->sendMessage($member['openid'], array('nickname' => $member['nickname'], 'ordersn' => $order['ordersn'], 'price' => $pricetotal, 'goods' => $goods, 'commission1' => $commission_total1, 'commission2' => $commission_total2, 'commission3' => $commission_total3, 'finishtime' => $order['finishtime']), TM_COMMISSION_ORDER_FINISH);
					}
					else {
						if ($order['agentid'] == $parent['id']) {
							$this->sendMessage($parent['openid'], array('nickname' => $member['nickname'], 'ordersn' => $order['ordersn'], 'price' => $pricetotal, 'goods' => $goods, 'commission1' => $commission_total1, 'commission2' => $commission_total2, 'commission3' => $commission_total3, 'finishtime' => $order['finishtime']), TM_COMMISSION_ORDER_FINISH);
						}
					}
				}
			}
		...
		protected function sendMore($touser, $tm, $tag, $msg, $datas)
		{
			...
			if (!empty($set['selfbuy']) && (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay'))) {
				...
				$msgbk = $msg;
				$msgbk['keyword2']['value'] = str_replace('[下线]', '下线', $msgbk['keyword2']['value']);
			...
			array_walk($res, function($value, $key) use($msg, $set, $datas, $tm, $tag) {
				if ($key == 0) {
					$msg['keyword2']['value'] = str_replace('[下线]', '二级下线', $msg['keyword2']['value']);
					if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) {
						$msg['keyword2']['value'] = str_replace('[佣金金额]', $datas['commission2'], $msg['keyword2']['value']);
					}
				}

				if ($key == 1) {
					$msg['keyword2']['value'] = str_replace('[下线]', '三级下线', $msg['keyword2']['value']);
					if (($tag == 'commission_order_finish') || ($tag == 'commission_order_pay')) {
						$msg['keyword2']['value'] = str_replace('[佣金金额]', $datas['commission3'], $msg['keyword2']['value']);
					}
				}

				if (!empty($tm[$tag]) && !empty($tm['templateid'])) {
					m('message')->sendTplNotice($value['openid'], $tm['templateid'], $msg);
					return NULL;
				}

				m('message')->sendCustomNotice($value['openid'], $msg);
			});
		}
		...
		protected function sendMoreAdvanced($touser, $tm, $tag, $msg, $url, $datas)
		{
			...
			foreach ($res as $key => $value) {
				if ($key == 0) {
					$msgbk = $msg;
					$msgbk = $this->replaceArray($msgbk, '下线', '二级下线');
					if (($tag == 'commission_order_finish_advanced') || ($tag == 'commission_order_pay_advanced')) {
				...
				if ($key == 1) {
					$msgbk = $msg;
					$msgbk = $this->replaceArray($msgbk, '下线', '三级下线');
					if (($tag == 'commission_order_finish_advanced') || ($tag == 'commission_order_pay_advanced')) {
