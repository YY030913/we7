...
class Rank_EweiShopV2Page extends CommissionMobilePage
{
	...
	public function main()
	{
		...
		case '1':
			...
			$paiming = 0;
			array_walk($result, function($val, $key) use(&$paiming, $user) {
				if ($val['mid'] == $user['id']) {
					$paiming += $key + 1;
				}
			});
	...
	protected function commissionVirtual($pindex, $psize)
	{
		...
		if (!empty($list)) {
			...
			$list = array_map(function($val) {
				$val['commission_total'] = number_format($val['commission_total'], 2);
				return array('commission_total' => $val['commission_total'], 'nickname' => $val['nickname'], 'avatar' => $val['avatar']);
			}, $list);
