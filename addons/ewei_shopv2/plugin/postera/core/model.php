...
class PosteraModel extends PluginModel
{
	...
	public function getQR($poster, $member)
	{
		...
		if (empty($qr)) {
			...
			$qr = array('acid' => $acid, 'openid' => $member['openid'], 'sceneid' => $scene_id, 'type' => $poster['type'], 'ticket' => $result['ticket'], 'qrimg' => $qrimg, 'posterid' => $poster['id'], 'expire' => $expire, 'url' => $result['url'], 'goodsid' => $poster['goodsid'], 'endtime' => $posterendtime);
			...
			if ($qr['endtime'] < time()) {
				...
				pdo_update('ewei_shop_postera_qr', array('ticket' => $ticket, 'qrimg' => $qrimg, 'url' => $result['url'], 'endtime' => $posterendtime), array('id' => $qr['id']));
				$qr['ticket'] = $ticket;
