...
class Single_EweiShopV2Page extends PluginWebPage
{
	...
	public function changestate()
	{
		...
		if ($_W['ispost']) {
			...
			foreach ($arr as $i => $data) {
				...
				$ordergood = pdo_fetch('SELECT id,goodsid,printstate,printstate2 FROM ' . tablename('ewei_shop_order_goods') . ' WHERE goodsid=:goodsid and orderid=:orderid and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':goodsid' => $ordergoodid, ':uniacid' => $_W['uniacid']));
	...
	public function getorderinfo()
	{
		...
		if ($_W['ispost']) {
			...
			$orders = pdo_fetchall('SELECT id,ordersn,address,address_send,status,paytype,expresscom,expresssn,dispatchtype FROM ' . tablename('ewei_shop_order') . ' WHERE id in( ' . $in . ' ) and (status=1 or (paytype=3 and status=0)) and uniacid=:uniacid order by ordersn desc ', array(':uniacid' => $_W['uniacid']));
	...
	public function dosend()
	{
		...
		if ($_W['ispost']) {
			...
			$orderinfo = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_order') . ' WHERE id=:orderid and status>-1 and uniacid=:uniacid limit 1', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
