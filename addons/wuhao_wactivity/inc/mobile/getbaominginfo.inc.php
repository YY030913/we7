...
$data_baoming=pdo_fetchall("SELECT * FROM ".tablename('wactivity_baoming')." WHERE `uniacid`=:uniacid AND `matchid`=:matchid AND `baomingstatus`=:baomingstatus",array(':uniacid'=>$_W['uniacid'],':matchid'=>$matchid,':baomingstatus'=>'报名'));
$data_total=array();
...
if($data_total){	
	...
	message('尚没有签到数据', $this->createMobileUrl('baoming', array()), 'error');
}
