...
class walletWeixinUtil
{
     ...
     * @return 带上签名的xmlData
     */
    public static function _getSign($xmlData,$keyss=''){
        ...
        $stringSignTemp="{$stringA}key=". $keyss;
    ...
    public  function getSendRedpackXml($inputObj){
        $xml = <<<eof
            <xml>
                ...
                <mch_id>{$inputObj->get_mchid()}</mch_id>
...
eof;
        $newXmlData = walletWeixinUtil::_getSign($xml,$inputObj->get_key());
