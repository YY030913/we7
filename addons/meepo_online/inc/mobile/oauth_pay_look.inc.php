...
if($type=='oauth_pay'){
    ...
    $params = @json_decode(base64_decode($sl) , true);
		...
		if ($_GPC['done'] == '1') {
				$sql = 'SELECT * FROM ' . tablename($this->online_pay_record) . ' WHERE `id`=:id';
				$log = pdo_fetch($sql, array(
						':id' =>$pay_id 
				));
				if(empty($log)){
					exit('错误订单'.$pay_id.'不存在或是已经被删除！');
				}
    ...
    if ($auth != $_GPC['auth']) {
		...
		$wOpt = $this->wechat_oauth_look_build($params, $wechat);
		if (is_error($wOpt)) {
			//pdo_delete($this->online_pay_record,array('id'=>$pay_id));
			die($wOpt['message']);
		}
