...
if($_W['isajax']){
	...
	if(empty($listid)){
		...
		$pingluns = array();
		$had_nums = intval($_GPC['had_nums']);
		$max_id = intval($_GPC['max_id']);
		$condition = '';
		if($max_id >0 ){
				$condition .= " AND id < '{$max_id}'";
		}
		...
		$psize = $num-$had_nums;
		if($psize > 0){
			$lists = pdo_fetchall("SELECT * FROM ".tablename($this->pinglun_table)." WHERE weid=:weid AND listid=:listid  AND status=:status $condition ORDER BY createtime DESC LIMIT ".$had_nums.",8",array(':weid'=>$weid,':listid'=>$listid,':status'=>'1'));
		...
		if(is_array($lists) && !empty($lists)){
				foreach($lists as $row){
					...
					if($row['type']=='redpack'){
						...
						$userinfo = "'".$row['avatar']."','".$row['nickname']."','".$row['sex']."','".$row['nickname']."'";
						$comment_html .= 
						'<li class="comment clearfix"><img src="'.$row['avatar'].'" class="cmt-ava">
							<p class="cmt-info" onclick="userinfo('.$userinfo.')">
