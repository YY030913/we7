...
if($op=='list'){
	...
	$paras = array(':weid'=>$weid,':listid'=>$listid,':status'=>$status,':type'=>'redpack');
	$total_success_money = pdo_fetchcolumn("SELECT SUM(money) FROM ".tablename($this->pinglun_table)." WHERE  weid=:weid AND listid=:listid AND status=:status AND type=:type",array(':weid'=>$weid,':listid'=>$listid,':status'=>1,':type'=>'redpack'));
	$total_fail_money = pdo_fetchcolumn("SELECT SUM(money) FROM ".tablename($this->pinglun_table)." WHERE  weid=:weid AND listid=:listid AND status=:status AND type=:type",array(':weid'=>$weid,':listid'=>$listid,':status'=>0,':type'=>'redpack'));
	...
	if(empty($list)){
		message('此项不存在或是已经被删除',$this->createWebUrl('dashang',array('listid'=>$listid,'status'=>$status)),'error');
	}else{
		pdo_update($this->pinglun_table,array('status'=>1),array('id'=>$id,'weid'=>$weid));
	}
	message('审核成功',$this->createWebUrl('dashang',array('listid'=>$listid,'status'=>'2')),'success');
}elseif($op=='cansay'){
	...
	message($message,$this->createWebUrl('dashang',array('listid'=>$listid,'status'=>'2')),'success');
...
if(checksubmit('down')){
			...
			$up_list = pdo_fetchall("SELECT * FROM ".tablename($this->pinglun_table)." WHERE  weid=:weid  AND listid=:listid AND status=:status AND type=:type AND id  IN  ('".implode("','", $_GPC['select'])."')", array(':weid'=>$weid,':listid'=>$listid,':status'=>$status,':type'=>'redpack'));
					...
					//导出
						...
						$objPHPExcel->getActiveSheet()->setCellValue('B1', '打赏金额');
						$objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(30);
						$objPHPExcel->getActiveSheet()->setCellValue('C1', '支付状态');
						$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(20);
						$objPHPExcel->getActiveSheet()->setCellValue('D1', '打赏时间');
						$objPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(20);
						$objPHPExcel->getActiveSheet()->setCellValue('E1', '打赏祝福');
						...
						foreach ($up_list as $key => $value) {
							...
							$objPHPExcel->getActiveSheet()->setCellValue('E'.($key+2),$value['content']);
						}
						...
						header('Content-Disposition:attachment;filename="打赏会员名单.xls"');
...
if(checksubmit('downall')){
			...
			$up_list = pdo_fetchall("SELECT * FROM ".tablename($this->pinglun_table)." WHERE  weid=:weid  AND listid=:listid AND status=:status AND type=:type", array(':weid'=>$weid,':listid'=>$listid,':status'=>$status,':type'=>'redpack'));
					...
					//导出
						...
						header("Content-Type:application/download");
						if($status=='1'){
							header('Content-Disposition:attachment;filename="打赏成功会员名单.xls"');
						}else{
							header('Content-Disposition:attachment;filename="打赏失败会员名单.xls"');
						}
						header("Content-Transfer-Encoding:binary");
						$objWriter->save('php://output');
