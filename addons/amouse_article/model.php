...
function amouse_article_site_list($cid=0){
    ...
    $weid=$_W['uniacid'];
    if($cid>0){
        $list = pdo_fetchall("SELECT * FROM ".tablename('fineness_article')." WHERE weid={$weid} AND ccate={$cid}  ORDER BY displayorder DESC, createtime DESC ") ;
    }
    return $list;
}
...
function amouse_article_by_id($id=0){
    ...
    $weid=$_W['uniacid'];
    if($id>0){
        $article = pdo_fetch("SELECT * FROM ".tablename('fineness_article')." WHERE weid={$weid} AND id={$id}  ") ;
    }
    return $article;
}
...
function amouse_article_site_get_next($params = array()) {
    ...
    $result = pdo_fetch("SELECT * FROM ".tablename('fineness_article')." WHERE weid = '{$_W['uniacid']}' $condition  LIMIT 1");
