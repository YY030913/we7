...
if ($op == 'alist') {
    ...
    if ($articleid) {
        ...
        $item  = pdo_fetch("SELECT * FROM " . tablename('fineness_article') . " WHERE id = :id", array(
            ':id' => $articleid
        ));
    ...
    $id        = intval($_GPC['id']);
    $art       = pdo_fetch("SELECT * FROM " . tablename('fineness_article') . " WHERE id = :id", array(
        ':id' => $articleid
    ));
    $item      = pdo_fetch("SELECT * FROM " . tablename('fineness_admire') . " WHERE id = :id", array(
        ':id' => $id
    ));
} elseif ($op == 'delete') {
    ...
    $id   = intval($_GPC['id']);
    $temp = pdo_delete("fineness_admire", array(
        'id' => $id
    ));
    message('删除数据成功！', $this->createWebUrl('comment', array(
        'op' => 'list',
        'articleid' => $articleid
    )), 'success');
}
