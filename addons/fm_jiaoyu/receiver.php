...
class Fm_jiaoyuModuleReceiver extends WeModuleReceiver {
	...
	public function receive() {
		...
		if ($this->message['event'] == 'subscribe' && !empty($this->message['ticket'])) {
			//logging_run("subscribe:subscribe,ticket");
			...
			//logging_run("unsubscribe:zz");
			$row = pdo_fetch("SELECT  * FROM " . tablename($this->table_qrinfo) . " WHERE openid = '{$this->message['from']}' and weid={$_W['uniacid']}");
		
			pdo_delete($this->table_qrstat, array('id' => $row['id']));
