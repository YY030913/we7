 ...
 */
        ...
        if(!empty($userid['id'])){
			...
			$leave1 = pdo_fetchall("SELECT * FROM " . tablename($this->table_notice) . " where :schoolid = schoolid And :weid = weid And :type = type And :groupid = groupid ORDER BY createtime DESC", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
				 ':type' => 2,
				 ':groupid' => 1
			...
			$leave2 = pdo_fetchall("SELECT * FROM " . tablename($this->table_notice) . " where :schoolid = schoolid And :weid = weid And :type = type And :groupid = groupid ORDER BY createtime DESC", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
				 ':type' => 2,
				 ':groupid' => 3
			...
			$leave3 = pdo_fetchall("SELECT * FROM " . tablename($this->table_notice) . " where :schoolid = schoolid And :weid = weid And :type = type And :bj_id = bj_id ORDER BY createtime DESC", array(
		         ':weid' => $weid,
				 ':schoolid' => $schoolid,
				 ':type' => 1,
				 ':bj_id' => $student['bj_id']
