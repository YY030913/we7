 ...
 */
        ...
        $kcming = pdo_fetchall("SELECT * FROM " . tablename($this->table_tcourse) . " WHERE weid =  '{$_W['uniacid']}' AND schoolid ={$schoolid} ORDER BY id ASC, id DESC", array(':weid' => $_W['uniacid'], ':schoolid' => $schoolid), 'id');
        if (!empty($kcming)) {
            $child = '';
            foreach ($kcming as $pid => $pcate) {
                if (!empty($pcate['parentid'])) {
                    $child[$pcate['parentid']][$pcate['id']] = array($pcate['id'], $pcate['name']);
                }
            }
        ...
        if ($operation == 'post') {
            ...
            if (checksubmit('submit')) {
                ...
                message('修改成功！', $this->createWebUrl('kcbiao', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
            }
            ...
            $list = pdo_fetchall("SELECT * FROM " . tablename($this->table_kcbiao) . " WHERE weid = '{$_W['uniacid']}' AND schoolid ={$schoolid} $condition ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);

            $total = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename($this->table_kcbiao) . " WHERE weid = '{$_W['uniacid']}' AND schoolid ={$schoolid} $condition");
            ...
            foreach ($_GPC['idArr'] as $k => $id) {
                ...
                if (!empty($id)) {
                    ...
                    pdo_delete($this->table_kcbiao, array('id' => $id, 'weid' => $_W['uniacid']));
                    $rowcount++;
                }
            }
