 ...
 */
        ...
        if ($operation == 'display') {
				...
				$list = pdo_fetchall("SELECT * FROM ".tablename($this->table_news)." WHERE weid = '{$_W['uniacid']}' And schoolid = '{$schoolid}' And type = 'article' $condition ORDER BY displayorder DESC, id DESC LIMIT ".($pindex - 1) * $psize.','.$psize, $params);
            ...
            if (checksubmit('submit')) {
                ...
                message('发布成功！', $this->createWebUrl('article', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
            }
            ...
            message('文章删除成功！', $this->createWebUrl('article', array('op' => 'display', 'schoolid' => $schoolid)), 'success');
        }
