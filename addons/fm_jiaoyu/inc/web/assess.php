 ...
 */
        ...
        if ($operation == 'post') {
            ...
            foreach ($list as $key => $value) {
				$member = pdo_fetch("SELECT * FROM " . tablename ( 'mc_members' ) . " where uniacid = :uniacid And uid = :uid ORDER BY uid ASC", array(':uniacid' => $_W ['uniacid'], ':uid' => $value['uid']));
			...
			if (checksubmit('submit')) {
			   ...
			   pdo_delete($this->table_teachers, array('birthdate' => 0, 'sex' => 0, 'jiontime' => 0, 'weid' => $_W['uniacid'], 'schoolid' => $schoolid));
