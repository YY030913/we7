// JavaScript Document 
var _postLink= {
    _appDownload: _domain+"/download?from=post",
    _map: "http://map.baidu.com"
};
var _link= {
    _appIntro: _domain,
    _appDownload: _domain+"/download?from=detail",
    _downloadGuide: _domain+"/download_guide",
    _followMp: _domain+"/post",
    _hot: _domain+"find",
    _post: _domain+"/post",
    _login: _domain+"/login",
    _loginWithWeixin:_domain+ "/go_wechat_oauth",
    _loginWithQq: _domain+"/go_qq_oauth",
    _loginWithWeibo: _domain+"/go_weibo_oauth",
    _loginWithWeixinMp: _domain+"/login/mp",
    _loginBackDefault:_domain+ "/post",
    _alipay: _domain+"alipay_submit",
    _showMap: _domain+"/show_map",
    _checkOrder: _domain+"/pay_check_pay_order",
    _fieldCacheHandle: _domain+"field_cache_handle"
};
var _api3= {
    _infoHintShare: _domain+"/post/_infoHintShare",
    _like: _domain+"/post/_like",
    _guess: _domain+"/post/api:55",
    _likeList: _domain+"/post/_likeList",
    _reviewList: _domain+"/post/_reviewList",
    _review: _domain+"/post/_review",
    _joinList: _domain+"/post/_joinList",
    _joinProperty: _domain+"/post/api:56",
    _joinParty: _domain+"/post/_joinParty",
    _joinRecruit: _domain+"/post/api:40",
    _getQr: _domain+"/get/api:qr",
    _getQr2: _domain+"/get/api:qr2",
    _loginWithQr: _domain+"/post/api:24",
    _loginWithMp: _domain+"/post/api:6",
    _login: _domain+"/post/api:2",
    _getCode: _domain+"/post/api:4",
    _checkCode: _domain+"/post/api:3",
    _register: _domain+"/post/api:1",
    _registerNew: _domain+"/post/api:8",
    _vote: _domain+"/post/api:13",
    _downloadQr: _domain+"/get/api:qrdownload",
    _downloadJoinQr: _domain+"/get/api:joinqrdownload",
    _joinCancel: _domain+"/post/_joinCancel",
    _joinQr: _domain+"qrcode_sign_up",
    _shareCount: _domain+"/post/api:7",
    _timelineList: _domain+"/post/api:23",
    _checkInviteCode: _domain+"/get/api:45",
    _findList: _domain+"/post/api:60",
    _report: _domain+"/post/api:10",
    _feedback: _domain+"/post/api:35",
    _reset_from_phone: _domain+"/post/api:5",
    _reset:_domain+"/post/api:21",
    _checkPayOrder: _domain+"/post/api:62",
    _recommendList: _domain+"/post/api:83",
    _pc_recommendList: _domain+"/post/api:84",
    _activity_recommend:_domain+"/post/api:85",
    _delhd:_domain+"/post/api:86",
    _mess:_domain+"/post/api:90",
    _recommend:_domain+"/post/_recommend",
    _getWeiXinQr:_domain+"/post/api:93",
    _download:_domain+"/post/api:94",
    _stock:_domain+"/post/api:96",
    _isPayOrder:_domain+"/post/_isPayOrder",
    _cancelOrder:_domain+"/post/api:100",
    _leanCloud_chat:_domain+"/post/api:102",
    _timelineInfoList:_domain+"/post/api:106",
    _shopNoBindInfoList:_domain+"/post/api:107",
    _ifJoinZero:_domain+"/post/api:104",
    _shopHits:_domain+"/post/api:105",
    _saveShopInfo : _domain+"/post/api:110",
    _editShop : _domain+"/post/api:111",
    _ifPass : _domain+"/post/api:112",
    _pzImgUp :_domain+ "/post/api:img_base64",
    _footerInfo : _domain+"/post/api:123",
    _shopInfo : _domain+"/post/_shopInfo",
    _shopAttent : _domain+"/post/api:125",
    _verifyCode : _domain+"/post/api:129",
    _createPrepayOrder : _domain+"/post/api:130",
    _createSmsOrder :_domain+ "/post/api:131",
    _checkOrderIsPay :_domain+ "/post/api:132",
    _sponsorList : _domain+"/post/api:135",
    _editUserDetail :_domain+"/post/api:140"
};
/**跳转链接*/
var _g= function(url) {
    location.href = url;
...
if (_user._inHudongba() && _user._useIOs()) {
    var HudongbaJsBridge = {
        ...
        //跳转报名凭证
        goJoinQr : function(info_id, info_type, info_title){
            var params = encode.base64Encode("://goJoinQr::" + info_id + "::" + info_type + "::" + encode._deCode(info_title));
            _g("hudongba" + params);
        },
        //关注
        focusShop : function(shopId, focusStatus){
            var params = encode.base64Encode("://focusShop::" + shopId + "::" + focusStatus);
            _g("hudongba" + params);
        }
    };
}
...
$(document).ready(function() {
  ...
  //有footer_page_type参数 说明引用了 public <#--21.PC端公共灰色底部&登录弹窗--> 需要初始化底部栏数据：互动吧简介、友情链接、热门城市、友情链接
  ...
  $(".header_public_search #searchBtn").on("click", function(){
...
//异步ajax初始化底部栏数据：互动吧简介、友情链接、热门城市、友情链接
...
function getFooterInfoOk(obj){
    ...
    //互动吧简介
    if(obj.hdb_desc){
        var html = "<div class=\"footer_gray_li jj\"><span>互动吧简介：</span><p>" + obj.hdb_desc + "</p></div>"
