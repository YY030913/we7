...
var _detailApi3 = {
  _sendCode: _domain+"/post/api:66",
  _verifyCode: _domain+"/post/api:67",
  _care:_domain+ "/post/_care",
  _template: _domain+"/post/api:78"
};
/*滚动位置设置*/
var _scroollTop = {
    ...
    if (code == 200) {
      ...
      if (recommendLength > 0) {
        ...
        for (var index = 0; index < recommendLength; index++) {
          ...
          recommendHtml += "<li><a class='dt_guess_list party' href='" + recommendItem.infoId36 + "'>";
      ...
      if (hotLength > 0) {
        ...
        for (var index = 0; index < hotLength; index++) {
          ...
          hotHtml += "<li><a class='dt_guess_list party' href='" + hotItem.infoId36 + "'>";
          //加数据
          ...
          hotHtml += "<div class='dt_guess_list_title'><span>" + hotItem.infoTitle + "</span></div>";
            if(hotItem.infoStartTime && hotItem.infoStartTime != undefined){
                hotHtml += "<div class='find_main_time'><img alt='' src='"+_imgCdn+"/images/icon_time.png'><p>"+hotItem.infoStartTime+"</p></div>";
            }
            if(hotItem.infoLocation && hotItem.infoLocation != undefined){
                hotHtml += "<div class='find_main_address'><img alt='' src='"+_imgCdn+"/images/icon_addr.png'><p class='dt_guess_list_attr'>"+hotItem.infoLocation+"</p><p class='dt_guess_list_num'><span>" + hotItem.join + "</span> 报名</p></div>";
            }else{
...
var _joinBar = {
  ...
  _show: function(id) {
      ...
      //活动已结束的直接跳转电子票页
      if(_info._state == "8"){
          _joinAfter._goJoinQrPage();
          return;
      }

      //报名付费活动的直接跳转电子票
      if(_info._joinPayItemMoney != "" && parseFloat(_info._joinPayItemMoney) > 0){
          _joinAfter._goJoinQrPage();
          return;
      }
  ...
  _cancelOk: function(json, code) {
    ...
    if(_info._joinPayItemId!=""&&_info._joinPayItemId!="0"){//有收费项时的取消报名：名额控制
      ...
      if(($("#detail_pay_list_li_"+_info._joinPayItemId).find(".detail_pay_list_b").html()||"").trim()=="已卖完"){
    	$("#detail_pay_list_li_"+_info._joinPayItemId).find(".detail_pay_list_b").html('剩余<b>1</b>');
    	$("#li_pay_item_"+_info._joinPayItemId).find(".joinNum").html('剩余<span class="num">1</span>个名额')
        $("#li_pay_item_"+_info._joinPayItemId).removeClass("unable");
        $("#li_pay_item_"+_info._joinPayItemId).addClass("able");
...
var _joinAfter = {
  ...
  _goJoinQrPage : function(){
      if(_user._inHudongba() && _info._appVersion != "" && parseFloat(_info._appVersion) >= 5.1){
          ...
          _g(_domain + "paysuccess&infoId=" + _info._id);
      }
  }
};
  ...
  _checkOk2: function(json, code) {
    ...
    if (json.prderstate == "2") { //未锁定
      ...
      if (document.referrer.indexOf('login') != -1) {
    	...
    	dataForShare.title = "我报名了《" + dataForShare.title + "》，你也来看看吧"; //2015.06.24 邀请码功能已不再使用
        $("#dt_join_bar_title").html("<a class=\"zTai02\" href=\"javascript:void(0);\" ontouchstart\"\" onclick=\"_joinBar._show()\">已报名</a>");
        $("#dt_join_bar_title_mb").html("<a href=\"javascript:void(0)\" ontouchstart=\"\" onclick=\"_joinBar._show('${type}')\" class=\"btn_style2\"><p class=\"bar_img\"><span><img class=\"dt_join_bar_icon\" src=\"" + _imgCdn + "/images/icon_man2.png\" alt=\"\" /></span></p> <p class=\"bar_tit\"><span>已报名</span></p></a>");
...
function goMore() {
  if (_user._inHudongba() && _user._useIOs()) {
    ...
    window.location.href = _link._hot;
  }
}
