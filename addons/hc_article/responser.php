...
class QRResponser {
	public function aa($openid){
		...
		$qr_file = IA_ROOT."/addons/hc_article/style/poster/membershare/$imgname";
		if(!file_exists($qr_file)){
			require IA_ROOT . '/framework/library/qrcode/phpqrcode.php';
			$value = $_W['siteroot'] . 'app/index.php?i=' . $uniacid . '&c=entry&m=hc_article&do=index&mid=' . $id;
			$errorCorrectionLevel = "L";
			$matrixPointSize = "4";
			QRcode::png($value, $qr_file, $errorCorrectionLevel, $matrixPointSize);
		}
		$qr_file = $_W['siteroot']."/addons/hc_article/style/poster/membershare/$imgname";
		$poster = pdo_fetch('SELECT * FROM ' . tablename('hc_article_poster') . ' WHERE uniacid=:uniacid limit 1', array(':uniacid' => $uniacid));
				...
				if ($dat['type'] == 'img') {
					...
					if ($dat['type'] == 'qr') {
						...
						if ($dat['type'] == 'realname') {
							$target = $this->mergeText($target, $dat, $member['nickname']);
						}
					}
				}
			}
		...
		if(file_exists($target_file)){
			...
			if(!empty($media_id)){
				pdo_update('hc_article_member', array('media_id'=>$media_id, 'ischange'=>0, 'media_time'=>time()), array('id'=>$id));
	...
	public function mergeText($target, $alldata, $text){
		$font = IA_ROOT . '/addons/hc_article/style/font/qrcodefont/msyhbd.ttf';
